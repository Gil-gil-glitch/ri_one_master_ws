...
class TaskPlannerNode(Node):
    ...
    def process_person_profile(self, profile: Dict):
        ...
        if not name:
            self._log('Invalid profile: missing name', level='warn')
            return

        # Resolve track_id from pending learns if not provided
        if not track_id:
            if self._pending_learns:
                track_id = next(iter(self._pending_learns))
                self._log(f'Resolved track_id={track_id} from pending learns for "{name}"')
            else:
                self._log(f'No pending learn to associate "{name}" with', level='warn')
                return

        if True:  # always confirm when a name is provided
